...
public class ProcessingMachineMenu extends AbstractContainerMenu {
        ...
        for (int slot = 0; slot < inputCount; slot++) {
            ...
            addSlot(new SlotItemHandler(items, slot, startX + slot * 18, 35) {
                ...
                public boolean mayPlace(@NotNull ItemStack stack) {
                    return machine.isValidInput(index, stack);
                }
            });
        }
    ...
    public static ProcessingMachineMenu fromNetwork(int id, Inventory inventory, FriendlyByteBuf buffer) {
        if (!(inventory.player.level().getBlockEntity(buffer.readBlockPos()) instanceof ProcessingMachineBlockEntity machine))
            throw new IllegalStateException("Missing processing machine");
    ...
    public boolean stillValid(Player player) {
        return player.level().getBlockEntity(machine.getBlockPos()) == machine && player.distanceToSqr(machine.getBlockPos().getX() + .5D, machine.getBlockPos().getY() + .5D, machine.getBlockPos().getZ() + .5D) <= 64D;
    }
    ...
    public @NotNull ItemStack quickMoveStack(@NotNull Player player, int index) {
        ...
        if (index < machineSlots) {
            ...
            for (int i = 0; i < inputCount; i++)
                if (machine.isValidInput(i, source) && moveItemStackTo(source, i, i + 1, false)) {
                    moved = true;
                    break;
                }
    ...
    public int getEnergy() {
        return (data.get(0) & 0x7FFF) | ((data.get(1) & 0x7FFF) << 15);
    }
