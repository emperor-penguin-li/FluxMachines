...
public class ProcessingMachineBlockEntity extends BlockEntity implements MenuProvider {
    ...
    private final class MachineEnergyStorage extends EnergyStorage {
        private MachineEnergyStorage() { super(100_000, 4096, 0); }
        @Override public int receiveEnergy(int maxReceive, boolean simulate) { int n = super.receiveEnergy(maxReceive, simulate); if (n > 0 && !simulate) setChanged(); return n; }
    ...
    private final IItemHandler automation = new IItemHandler() {
        private int outputSlot() { return kind() == 2 ? 3 : 1; }
        private int inputSlots() { return kind() == 2 ? 3 : 1; }
        ...
        @Override public boolean isItemValid(int slot, ItemStack stack) { return slot < inputSlots() && isValidInput(slot, stack); }
    };
    ...
    public ProcessingMachineBlockEntity(BlockPos pos, BlockState state) { super(ModBlockEntities.PROCESSING_MACHINE.get(), pos, state); }
    public IItemHandler getInventory() { return inventory; }
    public ContainerData getData() { return data; }
    public int getMachineKind() { return kind(); }
    public int getOutputSlot() { return kind() == 2 ? 3 : 1; }
    public int getInputSlotCount() { return kind() == 2 ? 3 : 1; }
    ...
    private int getProcessDuration() { return findRecipe().map(MachineRecipe::duration).orElse(200); }
    public void insert(ItemStack held) { if (held.isEmpty()) return; for (int i = 0; i < (kind() == 2 ? 3 : 1); i++) if (inventory.getStackInSlot(i).isEmpty()) { inventory.setStackInSlot(i, held.copyWithCount(1)); held.shrink(1); return; } }
    public void giveOutput(Player player) { int slot = kind() == 2 ? 3 : 1; ItemStack out = inventory.extractItem(slot, 64, false); if (!out.isEmpty() && !player.addItem(out)) player.drop(out, false); }
    public static void serverTick(Level level, BlockPos pos, BlockState state, ProcessingMachineBlockEntity machine) {
        ...
        if (recipe != null && machine.canOutput(recipe.result()) && machine.energy.consume(cost)) {
            machine.progress++;
            if (machine.progress >= duration) { machine.process(recipe); machine.progress = 0; }
            machine.setChanged();
    ...
    @Override public void load(@NotNull CompoundTag tag) { super.load(tag); inventory.deserializeNBT(tag.getCompound("Inventory")); energy.deserializeNBT(tag.get("Energy")); progress = tag.getInt("Progress"); }
    @Override protected void saveAdditional(@NotNull CompoundTag tag) { super.saveAdditional(tag); tag.put("Inventory", inventory.serializeNBT()); tag.put("Energy", energy.serializeNBT()); tag.putInt("Progress", progress); }
    ...
    @Override public @Nullable AbstractContainerMenu createMenu(int id, @NotNull Inventory playerInventory, @NotNull Player player) {
